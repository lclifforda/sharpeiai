...
const Settings = () => {
  ...
  const [config, setConfig] = useState<PlatformConfiguration>(() => {
    const base = JSON.parse(JSON.stringify(DEFAULT_PLATFORM_CONFIG)) as PlatformConfiguration;
    base.branding = { ...brandingCtx.branding };
    return base;
  });
  const [activeSection, setActiveSection] = useState<SectionId>("profile");
  const [selectedAppType, setSelectedAppType] = useState(config.applicationTypes[0]?.id ?? "");
  ...
  const updateBranding = (patch: Partial<typeof config.branding>) => {
    setConfig((prev) => ({ ...prev, branding: { ...prev.branding, ...patch } }));
