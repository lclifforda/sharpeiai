...
import { useDashboardStats, useDashboardFunnel } from "@/hooks/queries";
...
const Dashboard = () => {
  ...
  // Derive portfolio overview from API stats
  const livePortfolioOverview = stats
    ? [
        {
          label: "Total Portfolio",
          value: formatCurrency(stats.applications.total_amount),
          subvalue: `${stats.applications.approved} funded`,
          change: `${stats.applications.this_month} this mo`,
        },
        {
          label: "Active Vendors",
          value: String(stats.vendors.active),
          subvalue: `${stats.vendors.total} total`,
          change: "",
        },
        {
          label: "Applications",
          value: String(stats.applications.total),
          subvalue: `${stats.applications.pending_review} pending`,
          change: `${stats.applications.approved} approved`,
        },
        {
          label: "Customers",
          value: String(stats.customers.total),
          subvalue: `${stats.customers.active} active`,
          change: "",
        },
        {
          label: "Approved Amount",
          value: formatCurrency(stats.applications.approved_amount),
          subvalue: `of ${formatCurrency(stats.applications.total_amount)}`,
          change: "",
        },
      ]
    : portfolioOverview;

  // Derive underwriting funnel from API funnel data
  const liveFunnel = funnel
    ? funnel.map((item: { status: string; count: number; amount: string }, i: number) => ({
  ...
  // Derive documentation stats from API stats
  const liveDocStats = stats
    ? [
        { label: "Total Documents", value: String(stats.documents.total) },
        { label: "Verified", value: String(stats.documents.verified) },
        { label: "Pending Review", value: String(stats.documents.pending) },
        { label: "Active Vendors", value: String(stats.vendors.active) },
      ]
    : documentationStats;
      ...
      <div className="p-6 space-y-6">
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {livePortfolioOverview.map((item) => (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="p-5 border-border lg:col-span-2">
              ...
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={liveFunnel} layout="vertical" margin={{ left: 20, right: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <RePieChart>
                    <Pie
                      ...
                      outerRadius={80}
                    >
                      {liveFunnel.map((entry: { stage: string; color: string }) => (
