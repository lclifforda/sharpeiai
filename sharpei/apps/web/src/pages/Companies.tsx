...
import { Building2, Search, Plus, MapPin, Users, FileText, TrendingUp, Upload, Loader2 } from "lucide-react";
...
const Companies = () => {
  ...
  const allCompanies: MappedCompany[] = useMemo(
    () =>
      (rawCustomers ?? []).map((c: any) => ({
        id: c.id,
        name: c.name ?? "",
        industry: c.industry ?? "—",
        location: c.form_data?.address ?? "—",
        representatives: 0,
        activeContracts: 0,
        revenue: "$0",
