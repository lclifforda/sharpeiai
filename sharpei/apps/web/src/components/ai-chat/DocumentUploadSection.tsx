...
import { Bot, Upload, FileCheck, File, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { MarkdownText } from "@/components/MarkdownText";
import type { ChatMessage, DocumentVerificationResult } from "./helpers";
...
interface DocumentUploadSectionProps {
  ...
  documentVerification: Record<string, DocumentVerificationResult>;
...
const DocumentUploadSection = React.memo(({
  message,
  requiredDocuments,
  uploadedDocs,
  ...
  if (message.type !== 'document_upload') return null;
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isUploaded ? (
                      ...
                      <div className="text-xs text-muted-foreground">
                        PDF, DOC, DOCX, JPG, PNG &bull; Max 10MB
                      </div>
                    )}
                  </div>
                  ...
                  <label htmlFor={`file-${doc.id}`} className="cursor-pointer">
                    ...
                    <input
                      ...
                      onChange={(e) => onFileUpload(doc.id, e.target.files?.[0] || null)}
                    />
                  </label>
                </div>
              </div>
